...
class KodiDevice(IKodiDevice):
            ...
            if self._players and len(self._players) > 0:
                ...
                if self._mpchc is not None and self._mpchc_ws is None:
                    ...
                    if _mpchc_vars is not None:
                        ...
                        _mpchc_dur = _mpchc_vars.duration // 1000
                        if self._media_position != _mpchc_pos or self._media_position_updated_at is None:
                            self._media_position = _mpchc_pos
                            updated_data[MediaAttr.MEDIA_POSITION] = _mpchc_pos
                            self._media_position_updated_at = datetime.datetime.now(datetime.timezone.utc)
                            updated_data["media_position_updated_at"] = self.media_position_updated_at
                        if self._media_duration != _mpchc_dur:
                            self._media_duration = _mpchc_dur
                            updated_data[MediaAttr.MEDIA_DURATION] = _mpchc_dur
                        if _mpchc_vars.state == STATE_PLAYING:
                            _mpchc_state = MediaStates.PLAYING
                        elif _mpchc_vars.state == STATE_PAUSED:
                            _mpchc_state = MediaStates.PAUSED
                        else:
                            _mpchc_state = MediaStates.ON
                        if self._attr_state != _mpchc_state:
                            self._attr_state = _mpchc_state
                            updated_data[MediaAttr.STATE] = _mpchc_state
                        _mpchc_vol = _mpchc_vars.volumelevel
                        if self._volume != _mpchc_vol:
                            self._volume = _mpchc_vol
                            updated_data[MediaAttr.VOLUME] = _mpchc_vol
                        _mpchc_muted = bool(_mpchc_vars.muted)
    ...
    def attributes(self) -> dict[str, Any]:
        ...
        attributes = {
            ...
            KodiSensors.SENSOR_PLAY_STATUS: self.sensor_play_status,
            KodiSelects.SELECT_SUBTITLE_STREAM: {
                SelectAttributes.CURRENT_OPTION: self.selector_subtitle_stream,
                SelectAttributes.OPTIONS: (
